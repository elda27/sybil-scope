...
class TestTracer:
    ...
    def test_trace_string_types(self):
        ...
        # Use string types
        with tracer.trace("agent", "start") as ctx:
    ...
    def test_explicit_parent_id(self):
        ...
        with tracer.trace(TraceType.AGENT, ActionType.START, parent_id=parent_id):
            ...
            child_id = tracer.log(TraceType.LLM, ActionType.REQUEST, parent_id=12345)
        
        events = backend.load()
        
        # Check agent has correct parent
        agent_event = next(e for e in events if e.type == TraceType.AGENT and e.action == ActionType.START)
