...
from sybil_scope import (
    ...
    InMemoryBackend, FileBackend,
    set_global_tracer, trace_function
)
...
def example_custom_backend():
    ...
    backend = FilteredFileBackend(
        filepath="filtered_traces.jsonl",
        excluded_types=[TraceType.LLM]
    )
...
async def example_async_tracing():
    ...
    # Start async agent
    agent_id = tracer.log(
        TraceType.AGENT, ActionType.START,
        name="AsyncAgent", args={"mode": "parallel"}
    )
    
    # Run multiple async operations in parallel
    async with tracer.trace(TraceType.AGENT, ActionType.PROCESS,
                           parent_id=agent_id,
        ...
        for i in range(3):
            async def llm_task(idx):
                llm_id = tracer.log(
                    TraceType.LLM, ActionType.REQUEST,
                    model="gpt-4", args={"prompt": f"Query {idx}"}
                )
                result = await async_llm_call(f"Query {idx}")
                tracer.log(
                    TraceType.LLM, ActionType.RESPOND,
                    parent_id=llm_id, response=result
                )
                return result
        ...
        for tool in ["search", "calculator"]:
            async def tool_task(tool_name):
                tool_id = tracer.log(
                    TraceType.TOOL, ActionType.CALL,
                    name=tool_name, args={"query": "test"}
                )
                result = await async_tool_call(tool_name, {"query": "test"})
                tracer.log(
                    TraceType.TOOL, ActionType.RESPOND,
                    parent_id=tool_id, result=result
                )
                return result
        ...
        # Log results
        tracer.log(
            TraceType.AGENT, ActionType.PROCESS,
...
def example_performance_monitoring():
    ...
    for event in events:
        if event.action in [ActionType.END, ActionType.RESPOND] and event.parent_id in event_pairs:
    ...
    for pair_id, pair in event_pairs.items():
        if pair["start"] and pair["end"]:
            ...
            print(f"{pair['start'].type.value} - {details.get('function', details.get('name', 'Unknown'))}: {duration:.3f}s")
...
def example_multimodal_tracing():
    ...
    with tracer.trace(TraceType.AGENT, ActionType.START,
        ...
        with tracer.trace(TraceType.LLM, ActionType.REQUEST,
                         model="gpt-4-vision",
                         args={"image_path": "/tmp/user_image.jpg",
                               "prompt": "Describe this image"}) as vision_ctx:
        ...
        with tracer.trace(TraceType.TOOL, ActionType.CALL,
                         name="text_to_speech",
                         args={"text": "The image shows a sunset over mountains",
                               "voice": "natural"}) as tts_ctx:
            time.sleep(0.1)
            tracer.log(TraceType.TOOL, ActionType.RESPOND,
                      parent_id=tts_ctx.id,
                      name="text_to_speech",
                      result={"audio_path": "/tmp/description.mp3",
                              "duration": 3.5})
