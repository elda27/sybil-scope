...
from sybil_scope import Tracer, TraceType, ActionType, FileBackend
...
def generate_complex_agent_traces():
    ...
    user_id = tracer.log(
        TraceType.USER, ActionType.INPUT,
    ...
    with tracer.trace(TraceType.AGENT, ActionType.START, 
                      ...
                      task="Handle multi-part user request") as main_agent:
        
        # Planning phase
        with tracer.trace(TraceType.AGENT, ActionType.PROCESS,
                         label="Planning",
                         args={"user_query": "weather + news request"}) as planning:
            
            # LLM call for planning
            with tracer.trace(TraceType.LLM, ActionType.REQUEST,
                             model="gpt-4",
                             args={"prompt": "Plan how to handle weather and news request",
                                   "temperature": 0.2}) as llm_planning:
        ...
        with tracer.trace(TraceType.AGENT, ActionType.START,
                         name="WeatherAgent",
                         args={"location": "Tokyo"}) as weather_agent:
            
            # Weather API call
            with tracer.trace(TraceType.TOOL, ActionType.CALL,
                             name="weather_api",
                             args={"city": "Tokyo", "units": "metric"}) as weather_tool:
        ...
        with tracer.trace(TraceType.AGENT, ActionType.START,
                         name="NewsAgent", 
                         args={"topic": "AI news", "timeframe": "today"}) as news_agent:
            
            # First attempt - search fails
            with tracer.trace(TraceType.TOOL, ActionType.CALL,
                             name="news_search",
                             args={"query": "AI news today", "sources": ["techcrunch", "arxiv"]}) as news_tool1:
                time.sleep(0.08)
                tracer.log(TraceType.TOOL, ActionType.RESPOND,
            ...
            with tracer.trace(TraceType.TOOL, ActionType.CALL,
                             ...
                             args={"query": "artificial intelligence", "limit": 3}) as news_tool2:
                time.sleep(0.06)
                tracer.log(TraceType.TOOL, ActionType.RESPOND,
                          parent_id=news_tool2.id,
                          name="alternative_news_api",
                          result=[
                              {"title": "OpenAI releases new model", "summary": "Major breakthrough in reasoning"},
                              {"title": "AI ethics debate continues", "summary": "New regulations proposed"},
                              {"title": "Google AI advances", "summary": "Improved efficiency algorithms"}
                          ])
            
            # Summarize news
            with tracer.trace(TraceType.LLM, ActionType.REQUEST,
                             model="gpt-3.5-turbo",
                             args={"prompt": "Summarize these AI news articles",
                                   "max_tokens": 150}) as news_llm:
                time.sleep(0.04)
                tracer.log(TraceType.LLM, ActionType.RESPOND,
        ...
        with tracer.trace(TraceType.LLM, ActionType.REQUEST,
                         model="gpt-4",
                         args={"prompt": "Combine weather and news into user-friendly response",
                               "temperature": 0.3}) as final_llm:
            time.sleep(0.07)
            tracer.log(TraceType.LLM, ActionType.RESPOND,
...
def generate_error_scenario_traces():
    ...
    with tracer.trace(TraceType.AGENT, ActionType.START,
                      parent_id=user_id,
                      name="MathAndFinanceAgent") as agent_ctx:
        
        # Math calculation - will fail
        with tracer.trace(TraceType.TOOL, ActionType.CALL,
                         name="math_calculator",
                         args={"operation": "sqrt", "value": -1}) as math_tool:
            time.sleep(0.02)
            tracer.log(TraceType.TOOL, ActionType.RESPOND,
        ...
        with tracer.trace(TraceType.TOOL, ActionType.CALL,
                         name="complex_math",
                         args={"operation": "complex_sqrt", "value": -1}) as complex_tool:
            time.sleep(0.01)
            tracer.log(TraceType.TOOL, ActionType.RESPOND,
        ...
        with tracer.trace(TraceType.TOOL, ActionType.CALL,
                         name="stock_api",
                         args={"symbols": ["AAPL", "GOOGL", "MSFT"]}) as stock_tool:
            time.sleep(0.05)
            tracer.log(TraceType.TOOL, ActionType.RESPOND,
        ...
        with tracer.trace(TraceType.TOOL, ActionType.CALL,
                         name="cache_lookup",
                         args={"key": "stock_prices", "max_age": "1h"}) as cache_tool:
            time.sleep(0.005)
            tracer.log(TraceType.TOOL, ActionType.RESPOND,
...
def generate_performance_test_traces():
    ...
    with tracer.trace(TraceType.AGENT, ActionType.START,
                      parent_id=user_id,
                      name="DataProcessingAgent") as agent_ctx:
        
        # Data loading (slow)
        with tracer.trace(TraceType.TOOL, ActionType.CALL,
                         name="data_loader",
                         args={"source": "big_dataset.csv", "size": "10GB"}) as loader:
        ...
        for i in range(3):
            with tracer.trace(TraceType.TOOL, ActionType.CALL,
                             name=f"processor_{i}",
                             args={"chunk": i, "rows": 333333}) as processor:
        ...
        with tracer.trace(TraceType.TOOL, ActionType.CALL,
                         name="aggregator",
                         args={"method": "sum", "group_by": "category"}) as aggregator:
            time.sleep(0.08)
            tracer.log(TraceType.TOOL, ActionType.RESPOND,
        ...
        with tracer.trace(TraceType.LLM, ActionType.REQUEST,
                         ...
                         args={"template": "executive_summary", "data": "aggregated_results"}) as report_llm:
            time.sleep(0.02)
            tracer.log(TraceType.LLM, ActionType.RESPOND,
        ...
        with tracer.trace(TraceType.TOOL, ActionType.CALL,
                         name="exporter",
                         args={"format": "pdf", "destination": "reports/"}) as exporter:
            time.sleep(0.01)
            tracer.log(TraceType.TOOL, ActionType.RESPOND,
