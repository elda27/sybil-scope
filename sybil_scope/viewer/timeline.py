...
class TimelineRenderer:
    ...
    def create_gantt_data(self) -> pd.DataFrame:
        ...
        for event in self.events:
            if event.action in [ActionType.END, ActionType.RESPOND]:
                if event.parent_id and event.parent_id in event_pairs:
                    ...
                    pair['end'] = event 
                    pair['duration'] = (event.timestamp - pair['start'].timestamp).total_seconds()
        ...
        for event_id, pair in event_pairs.items():
            ...
            if end_event:
                ...
                display_name = name or label or function or model or f"{start_event.type.value}_{start_event.action.value}"
        ...
        single_events = [e for e in self.events 
                        if e.action not in [ActionType.START, ActionType.REQUEST, ActionType.CALL, ActionType.END, ActionType.RESPOND]]
        ...
        for event in single_events:
            ...
            display_name = name or label or function or f"{event.type.value}_{event.action.value}"
    ...
    def render_swimlane_timeline(self):
        ...
        for i, event_type in enumerate(event_types, 1):
            ...
            times = [(e.timestamp - self.start_time).total_seconds() for e in type_events]
            names = [e.details.get("name", e.details.get("label", e.action.value)) for e in type_events]
    ...
    def render_performance_timeline(self):
        ...
        for event in self.events:
            if event.action in [ActionType.START, ActionType.REQUEST, ActionType.CALL]:
                ...
                end_events = [e for e in self.events 
                             if e.parent_id == event.id and 
                             e.action in [ActionType.END, ActionType.RESPOND]]
                ...
                if end_events:
                    ...
                    display_name = name or label or function or model or f"{event.type.value}"
                    
                    performance_data.append({
                        'Operation': display_name,
                        'Type': event.type.value,
                        'Duration': duration,
                        'Start_Time': (event.timestamp - self.start_time).total_seconds(),
                        'ID': event.id
                    })
