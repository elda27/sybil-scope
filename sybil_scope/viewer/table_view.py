...
class TableRenderer:
    ...
    def get_event_duration(self, event: TraceEvent) -> Optional[float]:
    ...
    def render_flat_table(self):
        ...
        # Display table
        display_columns = ["Icon", "Type", "Action", "Name/Label", "Timestamp", 
                          "Duration (s)", "Parent ID", "Response", "Error"]
    ...
    def render_hierarchical_table(self):
        ...
        def render_tree_table(event_id: int, level: int = 0):
            ...
            with col7:
                # Details button
                if st.button("Details", key=f"details_{event_id}", help="View event details"):
    ...
    def render_summary_statistics(self):
        ...
        for event_type, stats in type_stats.items():
            ...
            avg_depth = sum(stats["avg_depth"]) / len(stats["avg_depth"]) if stats["avg_depth"] else 0
            ...
            summary_data.append({
                ...
                "Error Rate": f"{(stats['has_errors']/stats['count']*100):.1f}%" if stats["count"] > 0 else "0%"
            })
